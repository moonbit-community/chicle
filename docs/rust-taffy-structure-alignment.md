...
- Track sizing now has an explicit track-state initialization phase (`GridTrackSizingState`) before intrinsic/flex/free-space resolution, reducing the old monolithic helper toward Rust taffy's staged `track_sizing_algorithm` flow.
