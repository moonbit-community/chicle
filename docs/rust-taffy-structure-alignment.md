...
This document records the intentional differences between `moon_taffy` and the pinned Rust upstream `taffy v0.5.2`, and the local file-structure mapping used to keep the MoonBit port navigable against the Rust reference.
...
## Major differences from Rust upstream

- Rust uses a trait-driven layout engine (`LayoutPartialTree`, `LayoutPartialTreeExt`, `TraverseTree`, `PrintTree`) that can run over custom tree implementations. MoonBit currently uses a concrete `TaffyTree` backed by an array of internal nodes.
- Rust has a first-class `Cache` model with final layout and measure cache slots integrated with dirty tracking. MoonBit uses transient per-compute caches because the measure callback is supplied per call and has no stable identity.
...
- Rust splits sizing types into `Dimension`, `LengthPercentage`, `LengthPercentageAuto`, `MinTrackSizingFunction`, and `MaxTrackSizingFunction`. MoonBit currently uses one broader `Dimension` enum for public style sizing values.
...
| `src/tree/traits.rs` | not ported as traits; concrete tree APIs live in `tree_taffy_tree.mbt` |
