...
- Track sizing now has explicit track-state initialization, intrinsic flexible-track sizing, shrink-to-minimum overflow, and definite free-space distribution phases (`GridTrackSizingState`, `apply_intrinsic_flexible_track_sizing`, `shrink_grid_tracks_to_minimum`, `distribute_grid_track_free_space`) before final clamping, reducing the old monolithic helper toward Rust taffy's staged `track_sizing_algorithm` flow.
