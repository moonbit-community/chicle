...
- Internal compute dispatch now returns `LayoutOutput` through the cached/uncached path, matching Rust taffy's `compute_cached_layout` result flow. Recursive child execution is routed through `TaffyView::perform_child_layout`, `TaffyView::compute_child_layout`, and `TaffyView::measure_child_size`, mirroring Rust taffy's `LayoutPartialTreeExt` boundary instead of exposing loose package-level wrapper functions.
