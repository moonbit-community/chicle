...
- Track sizing now has explicit track-state initialization and intrinsic flexible-track sizing phases (`GridTrackSizingState`, `apply_intrinsic_flexible_track_sizing`) before shrink/free-space resolution, reducing the old monolithic helper toward Rust taffy's staged `track_sizing_algorithm` flow.
