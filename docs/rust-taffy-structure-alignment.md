...
- `GridItem` now records whether it crosses flexible or intrinsic tracks in both axes after placement and track construction, matching Rust taffy's per-item track metadata. The max-content spanning-track adapters consume the inline-axis and block-axis flags before falling through to the existing track-sizing helper.
