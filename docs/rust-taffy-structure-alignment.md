...
- Track sizing now has explicit track-state initialization, intrinsic flexible-track sizing, and shrink-to-minimum overflow phases (`GridTrackSizingState`, `apply_intrinsic_flexible_track_sizing`, `shrink_grid_tracks_to_minimum`) before free-space resolution, reducing the old monolithic helper toward Rust taffy's staged `track_sizing_algorithm` flow.
