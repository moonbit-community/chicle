...
- Grid container sizing helpers now live in `compute_grid_container_sizing.mbt`, covering used track size summation, definite/auto border-box resolution, deferred percent-gap rerun decisions, and the full percent-gap rerun track-sizing pass outside the main grid orchestration body.
- The grid main algorithm no longer materializes `placed_row`, `placed_col`, or span arrays for normal layout flow. Track contribution and final placement code reads placement directly from `GridItem`, with derived arrays isolated behind the max-content spanning-track adapter that still calls the existing MoonBit track-sizing helper.
- `GridItem` now records whether it crosses flexible or intrinsic tracks in both axes after placement and track construction, matching Rust taffy's per-item track metadata. The max-content spanning-track adapter consumes the inline-axis flags before falling through to the existing track-sizing helper; block-axis flags are initialized for the next row-axis track-sizing refactor.
