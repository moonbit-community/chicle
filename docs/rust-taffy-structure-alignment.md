...
- Grid auto-placement now uses `GridItem` placement records as the mutable placement source of truth before deriving track-sizing arrays. Each `GridItem` carries the node id, source order, and a style snapshot, and owns placement participation/span helpers, matching Rust taffy's grid item staging model more closely than the previous scattered child lookups and parallel placement arrays.
